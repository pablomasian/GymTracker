...
export default function Following() {
    ...
    const [error, setError] = useState(''); // ← AÑADE ESTA LÍNEA
    ...
    useEffect(() => {
        ...
        const fetchFollowing = async () => {
            try {
                console.log('Fetching following list for user:', user.id);
                ...
                setError(`Error loading following list: ${err.message}`);
            } finally {
                setLoading(false);
            }
        };
    ...
    // Añade esta condición para mostrar errores
    if (error) return (
    ...
    return (
        <div style={{ padding: '24px', maxWidth: '480px', margin: '0 auto' }}>
            <div style={{ marginBottom: '24px', textAlign: 'center' }}>
                <h2 style={{ margin: 0, marginBottom: '12px' }}>Following</h2>
                <button
                    onClick={() => nav('/view-profile')}
                    className="btn btn-secondary btn-sm"
